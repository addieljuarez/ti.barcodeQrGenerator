...
#define TAG "ti.barcode"

using namespace v8;

static Persistent<Object> bindingCache;

static void Barcode_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Barcode.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::BarcodeBindings::lookupGeneratedInit(
...
static void Barcode_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Barcode_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Barcode_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::BarcodeBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry BarcodeBinding = {
	"ti.barcode",
	Barcode_init,
	Barcode_dispose
};
...
Java_ti_barcode_BarcodeBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.barcode", &BarcodeBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::BarcodeBindings::lookupGeneratedInit));
}
